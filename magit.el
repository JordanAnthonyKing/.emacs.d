...
(use-package magit
  ...
  (defun my-git-commit-check-ticket-prefix (force)
    "Ensure the commit message starts with 'XS-#####: '.
If FORCE is non-nil, allow bypassing the check."
    (let ((commit-message (buffer-substring-no-properties (point-min) (point-max))))
      (if (or force
              (string-match-p "^XS-[0-9]\\{5\\}: " commit-message))
          t
        (progn
          (message "Commit message must start with 'XS-#####: '. Use C-u C-c C-c to force.")
          nil))))

  ;; Add to `git-commit-finish-query-functions` to enable the check
  (add-hook 'git-commit-finish-query-functions #'my-git-commit-check-ticket-prefix))
